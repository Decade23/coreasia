...
/**
 * Tab Akses bersama Pengguna 360 dan Ruang 360 — siapa melihat data subjek
 * ini (admin_riwayat_akses,
...
const props = defineProps<{
  /** Pengguna atau ruang. */
  subjek: string
  /** Kalimat pembuka dan kosong (berbeda untuk orang dan ruang). */
  ket: string
  kosong: string
}>()
...
<template>
  <div ref="wadah" class="scroll-mt-40 space-y-4">
    <p class="text-sm text-[var(--ca-muted)]">{{ ket }}</p>
    <CashflowPanelTab :memuat="memuat" :galat="galat" :pesan-galat="pesanGalat" :ada-data="!!mentah" :kosong="!!mentah && !baris.length" :pesan-kosong="kosong">
