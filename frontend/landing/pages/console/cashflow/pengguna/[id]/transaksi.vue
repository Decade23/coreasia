...
/**
 * Pengguna 360 › Transaksi — ranah `transaksi` (T2): admin_transaksi_cari
 * mode pengguna (transaksi yang DICATAT subjek, selalu di dalam lingkup ruang
 * kasus), keyset (occurred_at, created_at, id) desc, TANPA note.
 *
 ...
 * nilainya). Jawaban disimpan per (kasus, lingkup, saringan, kursor) di
 * useCashflowKasus, jadi pindah tab/Back tidak memanggil ulang; semuanya
 * dibuang saat kasus habis.
 *
 * Catatan (T3) lewat CashflowTeksTerkunci — membuka catatan baris yang
 * diklik SAJA (satu id, satu baris audit baca_teks; temuan fe p2 #3:
 * minimisasi, "hanya bagian yang relevan").
...
const { id, namaRuang } = useCashflowPengguna()
...
watch(kunci, (k) => {
  ...
  muat(async () => { await kasus.muatData(k, kk => api.transaksiCari(kk.id, id.value, s, ks, PER)) })
}, { immediate: true })
...
const labelPencatat = (p: string | null) => (!p ? '—' : p === id.value ? tcf('laci.subjekIni') : p.slice(0, 8))
const pesanKosong = computed(() => {
  if (jumlahSaring.value) return tcf('umum.kosongSaring')
  if (!kasus.kasus.value?.ruang.length) return tcf('transaksi.tanpaRuang')
...
<template>
  <div ref="wadah" class="scroll-mt-40 space-y-4">
      ...
      <div v-if="mentah" class="space-y-4" :class="{ 'opacity-60': memuat }">
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="text-left text-xs uppercase tracking-wide text-[var(--ca-muted)]">
                ...
                <th class="hidden px-3 py-2 md:table-cell">{{ tcf('transaksi.ruang') }}</th>
            ...
            <tbody>
              ...
              >
                ...
                <td class="hidden px-3 py-1.5 md:table-cell">{{ namaRuang(t.ruangId) }}</td>
        ...
        <ul v-if="baris.length" class="space-y-2 sm:hidden">
          <CashflowBarisTransaksi
            v-for="(t, i) in baris" :key="t.id" :t="t" :ruang="namaRuang(t.ruangId)" :sorot="sorot === i"
            @buka="pilihBaris(t, i)"
          />
        </ul>
    ...
    <CashflowLaci :show="!!q.tx" :judul="tcf('laci.judul')" @close="tutupLaci">
      <CashflowLaciTransaksi v-if="q.tx" :key="q.tx" :tx="q.tx" :subjek="id" :nama-ruang="namaRuang" @pindah="bukaLaci" />
    </CashflowLaci>
  </div>
</template>
