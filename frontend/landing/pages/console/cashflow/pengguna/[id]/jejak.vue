...
/**
 * Pengguna 360 › Jejak — ranah `jejak` (T2): admin_jejak dengan aktor =
 * subjek, di ruang lingkup kasus. Server mengirim kunci kolom yang berubah
 * TANPA nilainya dan `ada_judul` saja; judul & nilai selisih = teks bebas T3
 * (CashflowTeksTerkunci jenis 'jejak', id bigint — baris yang diklik saja). Jeda tiba (jam server −
 * jam perangkat) ditandai bila ≥ 1 menit — itu antrean di HP.
 *
 * Saringan ?ruang&jenis&dari&sampai (replace), halaman ?kursor= (push).
 * Sasaran transaksi bertaut ke laci di tab Transaksi (?tx=), sasaran pengguna
 * (anggota.*) ke kepala T0 orangnya.
 */
definePageMeta({
  layout: 'console',
...
const saring = computed<SaringJejak>(() => ({
  ruang: q.value.ruang || null, jenis: q.value.jenis || null, dari: q.value.dari || null, sampai: q.value.sampai || null,
}))
const kursor = computed(() => kursorJejakDariUrl(q.value.kursor))
const awalanKunci = computed(() => (kasus.kunciKasus.value && kasus.punyaRanah('jejak') ? `${kasus.kunciKasus.value}|jejak|${JSON.stringify(saring.value)}|` : null))
...
watch(kunci, (k) => {
  ...
  muat(async () => { await kasus.muatData(k, kk => api.jejak(kk.id, id.value, s, ks, PER)) })
}, { immediate: true })
...
const pilihanRuang = computed(() => (kasus.kasus.value?.ruang ?? []).map(w => ({ id: w, nama: namaRuang(w) })))
const jumlahSaring = computed(() => [q.value.ruang, q.value.jenis, q.value.dari, q.value.sampai].filter(Boolean).length)
...
const tujuanSasaran = (b: ReturnType<typeof keJejakBaris>): string | null => {
  ...
  if (b.sasaran === 'transaksi') return `${dasar.value}/transaksi?tx=${b.sasaranId}`
  if (b.sasaran === 'pengguna' && b.sasaranId !== id.value) return `/console/cashflow/pengguna/${b.sasaranId}`
  return null
}
useCashflowPintasan([
...
const pesanKosong = computed(() => (jumlahSaring.value ? tcf('umum.kosongSaring') : !kasus.kasus.value?.ruang.length ? tcf('transaksi.tanpaRuang') : tcf('jejak.kosong')))
</script>
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="text-xs text-[var(--ca-muted)]">{{ namaRuang(b.ruangId) }}</span>
