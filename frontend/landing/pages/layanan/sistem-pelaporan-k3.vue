...
// Subjek formulir kontak memakai kunci `k3`, terdaftar di contact.form.subjects
...
const contactPath = '/contact?subject=k3'
...
useCoreSeo({
  title: t('services.sistemPelaporanK3.title') as string,
  description: t('services.sistemPelaporanK3.description') as string,
  path: '/layanan/sistem-pelaporan-k3',
})

useSchemaOrg([
  defineWebPage({
    name: t('services.sistemPelaporanK3.title') as string,
    description: t('services.sistemPelaporanK3.description') as string,
  }),
])
...
const pricingOffer = t('services.sistemPelaporanK3.pricing') as unknown as PricingBlock
  ...
  script: [
    {
      ...
      innerHTML: JSON.stringify({
        ...
        '@type': 'Service',
        name: t('services.sistemPelaporanK3.schema.serviceName') as string,
        serviceType: 'Custom Software Development',
        description: t('services.sistemPelaporanK3.description') as string,
        url: 'https://coreasia.id/layanan/sistem-pelaporan-k3',
        provider: {
          '@type': 'Organization',
    ...
    },
    {
      ...
      innerHTML: JSON.stringify({
        '@context': 'https://schema.org',
        '@type': 'FAQPage',
        mainEntity: ((t('services.sistemPelaporanK3.faq.items') as Array<{ question: string, answer: string }>) || []).map((item) => ({
          '@type': 'Question',
          name: item.question,
...
const problemItems = computed(() => (t('services.sistemPelaporanK3.problems.items') as KartuIkon[]) || [])
const outcomeItems = computed(() => (t('services.sistemPelaporanK3.outcomes.items') as KartuIkon[]) || [])
const scopeItems = computed(() => (t('services.sistemPelaporanK3.scope.items') as KartuIkon[]) || [])
const regulationItems = computed(() => (t('services.sistemPelaporanK3.regulation.items') as Array<Record<string, string>>) || [])
const pricingFactors = computed(() => (t('services.sistemPelaporanK3.pricing.factors') as KartuIkon[]) || [])
const caseStudyPoints = computed(() => (t('services.sistemPelaporanK3.caseStudy.points') as string[]) || [])
const relatedItems = computed(() => (t('services.sistemPelaporanK3.related.items') as KartuIkon[]) || [])
const faqItems = computed(() => (t('services.sistemPelaporanK3.faq.items') as Array<{ question: string, answer: string }>) || [])
</script>
      ...
      <div class="ca-container relative ca-section pt-6 sm:pt-8 lg:py-28">
        <div ref="heroContent" class="mx-auto max-w-4xl text-center">
          <span class="ca-kicker">
            <Icon name="lucide:hard-hat" class="h-3.5 w-3.5 ca-tone-gold" />
            {{ t('services.sistemPelaporanK3.kicker') }}
          </span>
          <h1
            class="mt-5 text-balance font-display text-4xl font-bold leading-[1.08] text-[var(--ca-text)] sm:text-5xl lg:text-[3.45rem]"
            v-html="t('services.sistemPelaporanK3.hero.title')"
          />
          <p class="ca-copy mx-auto mt-5 max-w-2xl">
            {{ t('services.sistemPelaporanK3.hero.subtitle') }}
          </p>
          <div class="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <NuxtLink :to="contactPath" class="ca-btn-primary">
              {{ t('services.sistemPelaporanK3.hero.ctaPrimary') }}
              <Icon name="lucide:arrow-right" class="h-4 w-4" />
            </NuxtLink>
            <NuxtLink to="/portfolio" class="ca-btn-secondary">
              {{ t('services.sistemPelaporanK3.hero.ctaSecondary') }}
            </NuxtLink>
            ...
            <a href="#dasar-aturan" class="ca-btn-secondary">
              {{ t('services.sistemPelaporanK3.hero.ctaTertiary') }}
            </a>
          </div>
        </div>
      </div>
    ...
    <section class="ca-section">
      <div class="ca-container">
        <div ref="problemsHeader" class="mb-8 text-center">
          <h2 class="ca-title">{{ t('services.sistemPelaporanK3.problems.title') }}</h2>
          <p class="ca-copy mx-auto mt-3 max-w-2xl">{{ t('services.sistemPelaporanK3.problems.subtitle') }}</p>
        </div>
    ...
    <section class="ca-section pt-0">
      <div class="ca-container">
        <div ref="outcomesHeader" class="mb-8 text-center">
          <h2 class="ca-title">{{ t('services.sistemPelaporanK3.outcomes.title') }}</h2>
          <p class="ca-copy mx-auto mt-3 max-w-2xl">{{ t('services.sistemPelaporanK3.outcomes.subtitle') }}</p>
        </div>
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
    ...
    <section id="lingkup" class="ca-section pt-0">
      <div class="ca-container">
        <div ref="scopeHeader" class="mb-8 text-center">
          <h2 class="ca-title">{{ t('services.sistemPelaporanK3.scope.title') }}</h2>
          <p class="ca-copy mx-auto mt-3 max-w-2xl">{{ t('services.sistemPelaporanK3.scope.subtitle') }}</p>
        </div>
    ...
    <section id="dasar-aturan" class="ca-section pt-0">
      <div class="ca-container">
        <div ref="regulationHeader" class="mb-8 text-center">
          <h2 class="ca-title">{{ t('services.sistemPelaporanK3.regulation.title') }}</h2>
          <p class="ca-copy mx-auto mt-3 max-w-2xl">{{ t('services.sistemPelaporanK3.regulation.subtitle') }}</p>
        </div>
        <div class="grid gap-4 md:grid-cols-2">
          <article v-for="(item, index) in regulationItems" :key="item.title" :ref="revealRef('fadeUp', index * 80)" class="ca-card p-5">
            ...
            <div class="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2">
              <a
                ...
                class="ca-link-accent inline-flex items-center gap-1.5 text-sm font-semibold"
              >
                <Icon name="lucide:external-link" class="h-3.5 w-3.5" />
                {{ item.sourceLabel }}
              </a>
              <!-- Kartu yang mengutip lebih dari satu peraturan membawa tautan
                   keduanya, supaya janji "periksa sendiri" tetap bisa ditepati. -->
              <a
                v-if="item.sourceUrl2"
        ...
        <div ref="regulationNote" class="ca-card-soft mt-4 p-5">
          <p class="text-sm leading-relaxed text-[var(--ca-subtle)]">
            {{ t('services.sistemPelaporanK3.regulation.disclaimer') }}
          </p>
        </div>
      </div>
    </section>
    ...
    <section id="paket-harga" class="ca-section pt-0">
      <div class="ca-container">
        <div ref="pricingHeader" class="mb-8 text-center">
          <h2 class="ca-title">{{ t('services.sistemPelaporanK3.pricing.title') }}</h2>
          <p class="ca-copy mx-auto mt-3 max-w-2xl">{{ t('services.sistemPelaporanK3.pricing.subtitle') }}</p>
        </div>
        <div ref="pricingCard" class="ca-card p-6 text-center sm:p-8">
          <h3 class="text-base font-display font-semibold text-[var(--ca-text)]">
            {{ t('services.sistemPelaporanK3.pricing.headline') }}
          </h3>
          <p class="mt-3 font-display text-3xl font-bold ca-tone-gold sm:text-4xl">
            {{ t('services.sistemPelaporanK3.pricing.range') }}
          </p>
          <p class="mt-2 text-sm text-[var(--ca-muted)]">
            {{ t('services.sistemPelaporanK3.pricing.duration') }}
          </p>
          <p class="mx-auto mt-4 max-w-2xl text-xs leading-relaxed text-[var(--ca-subtle)]">
            {{ t('services.sistemPelaporanK3.pricing.note') }}
          </p>
          <NuxtLink
            :to="t('services.sistemPelaporanK3.pricing.noteLinkTo') as string"
            class="ca-link-accent mt-3 inline-flex items-center gap-1.5 text-xs font-semibold"
          >
            {{ t('services.sistemPelaporanK3.pricing.noteLinkLabel') }}
            <Icon name="lucide:arrow-right" class="h-3.5 w-3.5" />
          </NuxtLink>
        </div>
        <h3 class="mt-8 mb-4 text-center text-base font-display font-semibold text-[var(--ca-text)]">
          {{ t('services.sistemPelaporanK3.pricing.factorsTitle') }}
        </h3>
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
    ...
    <section class="ca-section pt-0">
      <div class="ca-container">
        <div ref="relatedHeader" class="mb-8 text-center">
          <h2 class="ca-title">{{ t('services.sistemPelaporanK3.related.title') }}</h2>
          <p class="ca-copy mx-auto mt-3 max-w-2xl">{{ t('services.sistemPelaporanK3.related.subtitle') }}</p>
        </div>
    ...
    <section id="faq" class="ca-section pt-0">
      <div class="ca-container">
        <div ref="faqSection" class="ca-card p-6 sm:p-8">
          <h2 class="ca-title mb-6">{{ t('services.sistemPelaporanK3.faq.title') }}</h2>
          <div class="space-y-3">
      ...
      <div class="ca-container">
        <div ref="ctaSection" class="ca-card p-6 text-center sm:p-10">
          <h2 class="text-balance font-display text-3xl font-bold text-[var(--ca-text)] sm:text-4xl">
            {{ t('services.sistemPelaporanK3.cta.title') }}
          </h2>
          <p class="mx-auto mt-3 max-w-2xl text-sm text-[var(--ca-muted)] sm:text-base">
            {{ t('services.sistemPelaporanK3.cta.subtitle') }}
          </p>
          <div class="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
            <NuxtLink :to="contactPath" class="ca-btn-primary">
              {{ t('services.sistemPelaporanK3.cta.button') }}
              <Icon name="lucide:arrow-right" class="h-4 w-4" />
            </NuxtLink>
