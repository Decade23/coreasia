...
// Subjek formulir kontak memakai kunci `operasional`, terdaftar di contact.form.subjects
...
const contactPath = '/contact?subject=operasional'
...
useCoreSeo({
  title: t('services.sistemManajemenTransaksi.title') as string,
  description: t('services.sistemManajemenTransaksi.description') as string,
  path: '/layanan/sistem-manajemen-transaksi',
})

useSchemaOrg([
  defineWebPage({
    name: t('services.sistemManajemenTransaksi.title') as string,
    description: t('services.sistemManajemenTransaksi.description') as string,
  }),
])
...
const pricingOffer = t('services.sistemManajemenTransaksi.pricing') as unknown as PricingBlock
  ...
  script: [
    {
      ...
      innerHTML: JSON.stringify({
        ...
        '@type': 'Service',
        name: t('services.sistemManajemenTransaksi.schema.serviceName') as string,
        serviceType: 'Custom Software Development',
        description: t('services.sistemManajemenTransaksi.description') as string,
        url: 'https://coreasia.id/layanan/sistem-manajemen-transaksi',
        provider: {
          '@type': 'Organization',
    ...
    },
    {
      ...
      innerHTML: JSON.stringify({
        '@context': 'https://schema.org',
        '@type': 'FAQPage',
        mainEntity: ((t('services.sistemManajemenTransaksi.faq.items') as Array<{ question: string, answer: string }>) || []).map((item) => ({
          '@type': 'Question',
          name: item.question,
...
const problemItems = computed(() => (t('services.sistemManajemenTransaksi.problems.items') as KartuIkon[]) || [])
const outcomeItems = computed(() => (t('services.sistemManajemenTransaksi.outcomes.items') as KartuIkon[]) || [])
const scopeItems = computed(() => (t('services.sistemManajemenTransaksi.scope.items') as KartuIkon[]) || [])
const pricingFactors = computed(() => (t('services.sistemManajemenTransaksi.pricing.factors') as KartuIkon[]) || [])
const caseStudyPoints = computed(() => (t('services.sistemManajemenTransaksi.caseStudy.points') as string[]) || [])
const relatedItems = computed(() => (t('services.sistemManajemenTransaksi.related.items') as KartuIkon[]) || [])
const faqItems = computed(() => (t('services.sistemManajemenTransaksi.faq.items') as Array<{ question: string, answer: string }>) || [])
</script>
      ...
      <div class="ca-container relative ca-section pt-6 sm:pt-8 lg:py-28">
        <div ref="heroContent" class="mx-auto max-w-4xl text-center">
          <span class="ca-kicker">
            <Icon name="lucide:receipt" class="h-3.5 w-3.5 ca-tone-gold" />
            {{ t('services.sistemManajemenTransaksi.kicker') }}
          </span>
          <h1
            class="mt-5 text-balance font-display text-4xl font-bold leading-[1.08] text-[var(--ca-text)] sm:text-5xl lg:text-[3.45rem]"
            v-html="t('services.sistemManajemenTransaksi.hero.title')"
          />
          <p class="ca-copy mx-auto mt-5 max-w-2xl">
            {{ t('services.sistemManajemenTransaksi.hero.subtitle') }}
          </p>
          <div class="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <NuxtLink :to="contactPath" class="ca-btn-primary">
              {{ t('services.sistemManajemenTransaksi.hero.ctaPrimary') }}
              <Icon name="lucide:arrow-right" class="h-4 w-4" />
            </NuxtLink>
            <NuxtLink to="/portfolio" class="ca-btn-secondary">
              {{ t('services.sistemManajemenTransaksi.hero.ctaSecondary') }}
            </NuxtLink>
            ...
            <a href="#paket-harga" class="ca-btn-secondary">
              {{ t('services.sistemManajemenTransaksi.hero.ctaTertiary') }}
            </a>
          </div>
        </div>
      </div>
    ...
    <section class="ca-section">
      <div class="ca-container">
        <div ref="problemsHeader" class="mb-8 text-center">
          <h2 class="ca-title">{{ t('services.sistemManajemenTransaksi.problems.title') }}</h2>
          <p class="ca-copy mx-auto mt-3 max-w-2xl">{{ t('services.sistemManajemenTransaksi.problems.subtitle') }}</p>
        </div>
    ...
    <section class="ca-section pt-0">
      <div class="ca-container">
        <div ref="outcomesHeader" class="mb-8 text-center">
          <h2 class="ca-title">{{ t('services.sistemManajemenTransaksi.outcomes.title') }}</h2>
          <p class="ca-copy mx-auto mt-3 max-w-2xl">{{ t('services.sistemManajemenTransaksi.outcomes.subtitle') }}</p>
        </div>
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
    ...
    <section id="lingkup" class="ca-section pt-0">
      <div class="ca-container">
        <div ref="scopeHeader" class="mb-8 text-center">
          <h2 class="ca-title">{{ t('services.sistemManajemenTransaksi.scope.title') }}</h2>
          <p class="ca-copy mx-auto mt-3 max-w-2xl">{{ t('services.sistemManajemenTransaksi.scope.subtitle') }}</p>
        </div>
    ...
    <section id="paket-harga" class="ca-section pt-0">
      <div class="ca-container">
        <div ref="pricingHeader" class="mb-8 text-center">
          <h2 class="ca-title">{{ t('services.sistemManajemenTransaksi.pricing.title') }}</h2>
          <p class="ca-copy mx-auto mt-3 max-w-2xl">{{ t('services.sistemManajemenTransaksi.pricing.subtitle') }}</p>
        </div>
        <div ref="pricingCard" class="ca-card p-6 text-center sm:p-8">
          <h3 class="text-base font-display font-semibold text-[var(--ca-text)]">
            {{ t('services.sistemManajemenTransaksi.pricing.headline') }}
          </h3>
          <p class="mt-3 font-display text-3xl font-bold ca-tone-gold sm:text-4xl">
            {{ t('services.sistemManajemenTransaksi.pricing.range') }}
          </p>
          <p class="mt-2 text-sm text-[var(--ca-muted)]">
            {{ t('services.sistemManajemenTransaksi.pricing.duration') }}
          </p>
          <p class="mx-auto mt-4 max-w-2xl text-xs leading-relaxed text-[var(--ca-subtle)]">
            {{ t('services.sistemManajemenTransaksi.pricing.note') }}
          </p>
          <NuxtLink
            :to="t('services.sistemManajemenTransaksi.pricing.noteLinkTo') as string"
            class="ca-link-accent mt-3 inline-flex items-center gap-1.5 text-xs font-semibold"
          >
            {{ t('services.sistemManajemenTransaksi.pricing.noteLinkLabel') }}
            <Icon name="lucide:arrow-right" class="h-3.5 w-3.5" />
          </NuxtLink>
        </div>
        <h3 class="mt-8 mb-4 text-center text-base font-display font-semibold text-[var(--ca-text)]">
          {{ t('services.sistemManajemenTransaksi.pricing.factorsTitle') }}
        </h3>
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      ...
      <div class="ca-container">
        <div ref="caseStudySection" class="ca-card p-6 sm:p-8">
          ...
          <p class="mt-5 text-sm leading-relaxed text-[var(--ca-subtle)]">
            {{ t('services.sistemManajemenTransaksi.caseStudy.note') }}
          </p>
          <NuxtLink to="/portfolio" class="ca-link-accent mt-5 inline-flex items-center gap-1.5 text-sm font-semibold">
            {{ t('services.sistemManajemenTransaksi.caseStudy.ctaLabel') }}
            <Icon name="lucide:arrow-right" class="h-3.5 w-3.5" />
          </NuxtLink>
        </div>
      </div>
    ...
    <section class="ca-section pt-0">
      <div class="ca-container">
        <div ref="relatedHeader" class="mb-8 text-center">
          <h2 class="ca-title">{{ t('services.sistemManajemenTransaksi.related.title') }}</h2>
          <p class="ca-copy mx-auto mt-3 max-w-2xl">{{ t('services.sistemManajemenTransaksi.related.subtitle') }}</p>
        </div>
    ...
    <section id="faq" class="ca-section pt-0">
      <div class="ca-container">
        <div ref="faqSection" class="ca-card p-6 sm:p-8">
          <h2 class="ca-title mb-6">{{ t('services.sistemManajemenTransaksi.faq.title') }}</h2>
          <div class="space-y-3">
      ...
      <div class="ca-container">
        <div ref="ctaSection" class="ca-card p-6 text-center sm:p-10">
          <h2 class="text-balance font-display text-3xl font-bold text-[var(--ca-text)] sm:text-4xl">
            {{ t('services.sistemManajemenTransaksi.cta.title') }}
          </h2>
          <p class="mx-auto mt-3 max-w-2xl text-sm text-[var(--ca-muted)] sm:text-base">
            {{ t('services.sistemManajemenTransaksi.cta.subtitle') }}
          </p>
          <div class="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
            <NuxtLink :to="contactPath" class="ca-btn-primary">
              {{ t('services.sistemManajemenTransaksi.cta.button') }}
              <Icon name="lucide:arrow-right" class="h-4 w-4" />
            </NuxtLink>
