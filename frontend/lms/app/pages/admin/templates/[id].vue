    ...
    <DashboardLayout>
        <template #header>
            <div class="flex items-center gap-4 w-full">
                <NuxtLink to="/admin/templates" class="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-content-subtle hover:text-white hover:bg-white/10 transition-all shrink-0">
                    ...
                    <div class="ca-card p-0 overflow-hidden">
                        ...
                        <div class="p-6 space-y-4">
                            <div>
                                ...
                                <p class="text-sm font-bold text-white">{{ template.name }}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p class="text-sm font-bold text-white">{{ template.schemeName }}</p>
                            </div>
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <p class="text-xs text-content-subtle">Dibuat</p>
                                    <p class="text-sm text-white">{{ template.createdAt.toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-content-subtle">Diperbarui</p>
                                    <p class="text-sm text-white">{{ template.updatedAt.toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
