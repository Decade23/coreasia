    ...
    <div class="ca-card group flex flex-col md:flex-row md:items-start gap-6 relative overflow-hidden p-6 hover:border-brand/30">
        ...
        <div class="flex-1 min-w-0 z-10">
            <div class="flex flex-wrap items-center gap-2 mb-3">
                <BaseBadge :text="question.schemeName" variant="default" type="solid" class="bg-white/10" />
                <BaseBadge :text="typeLabel(question.type)" variant="default" type="outline" class="border border-white/10 text-slate-400" />
                <span
            ...
            <p class="text-white font-medium text-sm md:text-base leading-relaxed group-hover:text-brand-100 transition-colors">
                {{ question.text }}
            </p>
            ...
            <div v-if="question.type === 'multiple_choice' && question.options.length > 0" class="mt-3 space-y-1.5">
                ...
                >
                    <span class="w-5 h-5 rounded-full border text-[10px] font-black flex items-center justify-center shrink-0"
                        :class="opt.isCorrect ? 'border-brand bg-brand/20' : 'border-white/10'">
                        {{ opt.id }}
                    </span>
                    {{ opt.text }}
                </div>
