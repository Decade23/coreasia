...
<template>
  <div class="space-y-4 p-6 md:p-10 rounded-[2.5rem] glass-card">
    <div 
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div class="flex items-center gap-5">
          <div 
            ...
            :class="doc.uploadedFile ? 'bg-gradient-to-br from-emerald-500 to-emerald-400 text-slate-950' : 'bg-core-900 border border-white/5 text-content-muted group-hover:text-white'"
          >
            <FileCheck v-if="doc.uploadedFile" class="w-6 h-6" />
            <File v-else class="w-6 h-6" />
          </div>
          <div>
            <h3 class="font-bold text-white text-sm md:text-base group-hover:text-emerald-400 transition-colors">{{ doc.label }}</h3>
        ...
        <label v-else class="relative cursor-pointer group/btn w-full sm:w-auto mt-2 sm:mt-0">
          ...
          />
          <div class="flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-white/5 border border-white/5 text-content font-bold text-sm group-hover/btn:bg-brand group-hover/btn:border-transparent group-hover/btn:text-slate-950 transition-all w-full sm:w-auto hover:shadow-lg hover:shadow-brand/20">
