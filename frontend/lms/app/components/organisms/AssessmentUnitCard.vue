...
<template>
  <div class="bg-[#0F1423] rounded-[2rem] overflow-hidden transition-all duration-500 shadow-glow-base">
    <button 
      @click="isOpen = !isOpen"
    ...
    >
      <div class="flex items-center gap-6">
        ...
        <div>
          ...
          <h3 class="text-xl font-bold text-white leading-tight transition-colors">{{ unit.title }}</h3>
        </div>
      </div>
    ...
    <div v-if="isOpen" class="p-6 md:p-8 space-y-10 animate-fade-in bg-[#050814]/40">
      <div v-for="element in unit.elements" :key="element.id" class="space-y-6">
        <div class="flex items-center justify-center mb-8">
          <span class="text-[10px] font-black text-content-subtle uppercase tracking-widest text-center px-5 py-2 rounded-full bg-[#1A2235]">Elemen: {{ element.title }}</span>
        </div>
